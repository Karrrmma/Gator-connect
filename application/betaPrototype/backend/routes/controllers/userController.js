...
exports.canRegister = (req, res) => {
  ...
  const query =
    'SELECT * FROM User INNER JOIN Account ON User.user_id = Account.user_id WHERE User.sfsu_email = ? OR Account.username = ?';
  connection.query(query, [sfsu_email, username], (err, results) => {
    if (err) {
      console.error('Error checking email and username:', err);
      return res
        .status(500)
        .json({ error: 'Failed to check email and username' });
    }

    if (results.length === 0) {
      return res
        .status(200)
        .json({ message: "Email and username don't exist" });
    }

    const existingEmail = results.find(
      (user) => user.sfsu_email === sfsu_email
    );
