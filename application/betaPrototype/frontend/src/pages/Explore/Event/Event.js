...
function Event() {
  ...
  const filteredEvents = events.filter((event) => {
    ...
    if (filters.location && event.event_location !== filters.location)
      return false;
    if (
      filters.startDate &&
      new Date(event.event_time) < new Date(filters.startDate)
    )
      return false;
    if (
      filters.endDate &&
      new Date(event.event_time) > new Date(filters.endDate)
    )
      return false;
  ...
  const isEventLapsed = (event_time) => {
    //if the event has lapsed, then it will display something different
    ...
    <div className="content-wrapper">
      <div className="search-wrapper">
        ...
        >
          ...
          From enriching cultural showcases to exhilarating sports tournaments,
          there's something for everyone. Immerse yourself in the dynamic campus
          community and discover the wide array of opportunities to engage,
          learn, and connect.
        </p>
        ...
        <div className="search-container-tee">
          ...
          >
            ...
            <option value="Creative Arts Building">
              Creative Arts Building
            </option>
            <option value="Business Building">Business Building</option>
            <option value="Ethnic Studies and Psychology Building">
        ...
        {showCreateEventForm && (
          <CreateEventForm
            ...
            onClose={handleCloseCreateEventForm}
          />
        )}
      </div>
          ...
          filteredEvents.map((event, index) => (
            <div key={index} className="grid-item">
              ...
              {isEventLapsed(event.event_time) ? (
                <button className="route-button" disabled>
                  Lapsed Event
                </button>
              ) : (
                <button onClick={handleRSVPClick} className="route-button">
                  Register
                </button>
              )}
            </div>
      ...
      {showForm && (
        <RSVPForm onSubmit={handleSubmitRSVP} onClose={handleCloseForm} />
      )}
    </div>
  );
}
