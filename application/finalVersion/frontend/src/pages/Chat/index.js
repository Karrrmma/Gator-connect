        ...
        <div className="chat-container ">
          ...
          >
            {privateChats.map((chats, index) => (
              <SenderName
                senderID={chats.sender_id || chats.receiver_id}
                index={index}
              />
            ))}
            <p
...
function SenderName({ senderID }, { index }) {
  ...
  return (
    ...
    >
      ...
      <Link
        to={`/chatWindow/${userData.fullName}`}
...
function Head() {
  return (
    <div>
      <h1 style={{ fontWeight: 'bold', color: 'white', fontSize: '2.0rem' }}>
        PRIVATE CHAT
      </h1>
      <p style={{ color: 'grey', fontSize: '1.1rem', marginBottom: '1.5rem' }}>
        {' '}
        Ready to catch up with your friends? Dive into the chat here!
      </p>
    </div>
  );
}
...
function Bottom() {
  return (
    <div className="chat-body d-flex flex-column align-items-center">
      <Link
        to="/pubChat"
        className="btn btn-link rounded-pill text-decoration-none"
        style={{
      ...
      <p
        style={{ background: 'black', fontSize: '15px', marginBottom: '20px' }}
      >
        Want to chat to everyone? <br></br> Go to PUBLIC CHAT to connect more!
      </p>
    </div>
  );
}
