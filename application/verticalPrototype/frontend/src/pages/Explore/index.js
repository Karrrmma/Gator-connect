...
import foodVendorIcon from './foodvendor.png';
import transportationIcon from './transportation.png';
  ...
  return (
    <section style={containerStyle}>
      ...
      {routes.map((route, index) => (
        <Link
          key={route.text}
          to={`/explore/${route.path}`}
          style={itemStyle}
        >
          <div style={{ display: 'flex', alignItems: 'center'}}>
            <img src={route.icon} alt={route.text} style={{ width: '40px' }} />
            <div style={textStyle}>
              <h2 style={{ margin: '0', fontSize: '17px' }}>{route.text}</h2>
              <p style={{ margin: '5px 0', fontSize: '14px' }}>{route.description}</p>
            </div>
          </div>
          <div style={checkItOutStyle}>CHECK IT OUT</div>
        </Link>
      ))}
      <Link to="/home" style={{ ...checkItOutStyle, marginTop: '30px', backgroundColor: 'gray' }}>
        BACK HOME
      </Link>
    </section>
  );
